...
class MouseColor extends Component{
    ...
    capturarRatonLogica(x,y){
        var f1 = Math.trunc(x/window.innerWidth*255); 
        var f2 = Math.trunc(y/window.innerHeight*255); 
    ...
    capturarRaton(event){
        if(this.state.switx){
            var bgHex= this.capturarRatonLogica(event.pageX,event.pageY);
        ...
        return (
            <div className="parent-mouse-color">
                <div className="main-mouse-color"
                    ...
                    <ChildPanel style={this.state.color} 
                        methodFromParent={this.capturarRatonLogica}
                        onCapturarRaton={this.capturarRaton}/>
                </div>
            </div>
        );
    }
